...
class Select extends BaseComponent {
    ...
    stateClasses = {
        ...
        isOnTheLeftSide: 'is-one-the-left-side',
        isOnTheRightSide: 'is-one-the-right-side',
    }
    ...
    constructor(rootElement){
        ...
        setTimeout(this.fixDropdownPosition, 500 ) // для корректных рассчетов
    ...
    updateTabIndexes(
        isMobileDevice  = MatchMedia.mobile.matches   // проверка меньше ли экран 768px
    ) {
    ...
    onClick = (event) => {         // закрытие выпадающей менюшки
        const { target } = event
        const isButtonClick = target === this.buttonElement
        const isOutSideDropdownClick = target.closest(this.selectors.dropdown) !== this.dropdownElement // closest - поиск ближайшего родителя
    ...
    bindEvents(){
        ...
        this.originalControlElement('change', this.onOriginalControlChange)
